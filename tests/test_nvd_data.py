"""
Test integrity of data from the NVD stored in SQL database. The purpose of this is to reveal any areas where
data may need to be examined or patched. It is not necessarily indicative of process failures.
...
select_all = "SELECT count(*) FROM nvd_transformed_data"
...
class Test_NVD():
    ...
    def test_check_cve_format(self):
        ...
        with pool.connect() as db_conn:
            ...
            select_all_correct = 'SELECT count(*) FROM nvd_transformed_data WHERE cve_id LIKE "CVE%%"'
    ...
    def test_check_base_score_range(self):
        ...
        with pool.connect() as db_conn:
            ...
            select_all_range = "SELECT count(*) FROM nvd_transformed_data WHERE base_score BETWEEN 0 AND 10"
            total = db_conn.execute(select_all).fetchall()
    ...
    def test_check_metric_version(self):
        ...
        with pool.connect() as db_conn:
            # count of all records with correct range
            select_all_not_null = "SELECT count(*) FROM nvd_transformed_data WHERE metric_version IS NOT NULL"
            total = db_conn.execute(select_all).fetchall()
    ...
    def test_check_impact_range(self):
        ...
        with pool.connect() as db_conn:
            # count of all records with correct range
            select_all_impact_range = "SELECT count(*) FROM nvd_transformed_data WHERE impact_score BETWEEN 0 AND 10"
            total = db_conn.execute(select_all).fetchall()
    ...
    def test_vuln_status(self):
        ...
        with pool.connect() as db_conn:
            # count of all records with correct range
            select_all_not_null = "SELECT count(*) FROM nvd_transformed_data WHERE vuln_status IS NOT NULL"
            total = db_conn.execute(select_all).fetchall()
            correct = db_conn.execute(select_all_not_null).fetchall()
            # check
            assert total == correct
    ...
    def test_exploit_score_range(self):
        ...
        with pool.connect() as db_conn:
            # count of all records with correct range
            select_all_exploit_range = "SELECT count(*) FROM nvd_transformed_data WHERE exploitability_score BETWEEN 0 AND 10"
            total = db_conn.execute(select_all).fetchall()
